...
func (ChromedpImporter) Import(ctx context.Context, req Request) (Result, error) {
	...
	if err := chromedp.Run(browser, chromedp.Navigate(base.String()), chromedp.WaitReady("body", chromedp.ByQuery), chromedp.Sleep(time.Second)); err != nil {
		return Result{}, fmt.Errorf("open Item Banks: %w", err)
	}
	name := jsString(req.BankName)
	var found bool
	if err := chromedp.Run(browser, chromedp.Evaluate(bankExistsJS(name), &found)); err != nil {
	...
	if !found {
		...
		if err := chromedp.Run(browser, chromedp.WaitVisible("[role=dialog] input", chromedp.ByQuery)); err != nil {
			return Result{}, fmt.Errorf("wait for bank-name field: %w", err)
		}
		if err := chromedp.Run(browser, chromedp.SendKeys("[role=dialog] input", req.BankName, chromedp.ByQuery)); err != nil {
			return Result{}, fmt.Errorf("fill bank name: %w", err)
		}
		if err := chromedp.Run(browser, chromedp.Click("[role=dialog] input[type=checkbox]", chromedp.ByQuery)); err != nil {
			return Result{}, fmt.Errorf("share bank with course: %w", err)
		}
		if err := chromedp.Run(browser, clickTextInDialog("Create Bank"), chromedp.Sleep(500*time.Millisecond)); err != nil {
			return Result{}, fmt.Errorf("submit create bank: %w", err)
		}
		if err := chromedp.Run(browser, chromedp.Navigate(base.String()), chromedp.WaitReady("body", chromedp.ByQuery)); err != nil {
			return Result{}, fmt.Errorf("return to Item Banks: %w", err)
		}
	}
	if err := chromedp.Run(browser, clickText(req.BankName)); err != nil {
		return Result{}, fmt.Errorf("open Item Bank %q: %w", req.BankName, err)
	}
	if err := chromedp.Run(browser, chromedp.WaitVisible(`button[aria-haspopup="true"]`, chromedp.ByQuery)); err != nil {
		return Result{}, fmt.Errorf("wait for Item Bank actions: %w", err)
	}

	if err := chromedp.Run(browser, chromedp.Click(`button[data-popover-trigger="true"]`, chromedp.ByQuery)); err != nil {
		return Result{}, fmt.Errorf("open import actions: %w", err)
	}
	if err := chromedp.Run(browser, chromedp.Click(`[role="menuitem"]`, chromedp.ByQuery)); err != nil {
		return Result{}, fmt.Errorf("open import dialog: %w", err)
	}
	if err := chromedp.Run(browser, chromedp.SetUploadFiles("input[type=file]", []string{req.Package}, chromedp.ByQuery)); err != nil {
	...
	if err := chromedp.Run(browser, chromedp.Poll(`document.body.innerText.includes('has imported successfully!')`, nil, chromedp.WithPollingTimeout(60*time.Second))); err != nil {
		return Result{}, fmt.Errorf("wait for import completion: %w", err)
	}
	var location string
	if err := chromedp.Run(browser, chromedp.Location(&location)); err != nil {
